...
[NOTE]
====
As of version 3 of OpenShfit Service Mesh, ingress & egress gateway creation and management will be decoupled from the `ServiceMeshControlPlane`, aligning it with upstream Istio.
===
...
As the Travel Services Domain Owner (Tech Lead) you have been given permissions to create a new gateway to expose services from the `{openshift_cluster_user_name}-prod-travel-agency` namespace. Login with credentials `farid/farid`.
...
4. If the JWT token is valid and the `AuthorizationPolicy` matches, the external user is allowed to access the `/PATH` - otherwise, an error message is returned to the user (for example, code `403`, message `RBAC: access denied`).
