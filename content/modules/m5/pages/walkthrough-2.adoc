...
* Install the helm repository and helm chart together with the values from the previous step.
+
[source,shell,subs=attributes,role=execute]
----
...
* Update the `{openshift_cluster_user_name}-production` `ServiceMeshControlPlane`
+
...
** Use the following command to restart the _controlplane_ pods (The Operator will automatically recreate the pods after they have been deleted):
+
[source,shell,subs=attributes,role=execute]
----
...
* Check for the Issuer on the certificates used in the communication with _istiod_, it should be `issuer=O = travelagency.com, CN = istio-ca.travelagency.com`.
+
[source,shell,subs=attributes,role=execute]
----
